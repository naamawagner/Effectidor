...
def effectors_learn(error_path, ORFs_file, effectors_file, working_directory, tmp_dir, queue, gff_file, full_genome_f,
    ...
    import sys
    sys.path.append(CONSTS.EFFECTIDOR_AUXILIARIES)
    ...
    T3Es_recs = SeqIO.parse(effectors_prots,'fasta')
    if len(list(T3Es_recs))>150:
        error_msg = 'the number of effectors found based on homology exceeds the number of expected T3Es in a single genome! If you included T3Es for homology search, make sure they contain nothing but true T3Es.'
    ...
    with open(f'{working_directory}/features_jobs.cmds','w') as jobs_f:
        # sequence features
        jobs_f.write(f'''{CONSTS.MODULE_LOAD}; python {scripts_dir}/sequence_features.py {ORFs_file} {effectors_prots} {working_directory}!@#\
                        touch {os.path.join(tmp_dir,"physical_features.done")}\tsequence_features\n''')
        # sequence similarity features
        jobs_f.write(f'''{CONSTS.MODULE_LOAD}; python {scripts_dir}/homology.py {ORFs_file} {all_prots} {blast_datasets_dir} {working_directory}!@#\
                        touch {os.path.join(tmp_dir,"homology_features.done")}\thomology\n''')
        if gff_file:
            # genome organization features
            jobs_f.write(f'{CONSTS.MODULE_LOAD}; python {scripts_dir}/genome_organization.py {ORFs_file} {effectors_prots} {working_directory} {gff_file}!@#\
    ...
    if signalp:
        with open(f'{working_directory}/signalp.cmds','w') as sig_f:
            sig_f.write(f'module load signalp6-6.0; {CONSTS.MODULE_LOAD}; {CONSTS.EFfECTIDOR_SCRIPTS}/effectidor_env/bin/python {scripts_dir}/signal_peptide_features.py {ORFs_file} {all_prots} {working_directory}!@#\
        ...
        if not os.path.exists(f'{working_directory}/signal_p_features.done'):
            subprocess.call(f'python {os.path.join(scripts_dir, "auxiliaries", "q_submitter_power.py")} --cpu 3 {working_directory}/signalp.cmds {tmp_dir} -q {queue}', shell=True)
    if not os.path.exists(f'{working_directory}/physical_features.done'):
        subprocess.call(f'python {os.path.join(scripts_dir, "auxiliaries", "q_submitter_power.py")} {working_directory}/features_jobs.cmds {tmp_dir} -q {queue} --memory 4', shell=True)
...
if __name__ == '__main__':
        ...
        parser.add_argument('--queue', default='pupko-pool')
