...
with open(os.path.join(working_directory, 'physical_features1.csv'), 'w', newline='') as file:
    ...
    for locus in locus_dic:
        l = [locus, GC_content(locus_dic[locus]), len(locus_dic[locus].translate(to_stop=True))]
...
features = pd.read_csv(os.path.join(working_directory, 'physical_features1.csv'))
features['GC_content(z_score)'] = stats.zscore(features['GC_content'])
features.drop(columns='GC_content', inplace=True)
features.to_csv(os.path.join(working_directory, 'physical_features.csv'), index=False)
        
endfile = open(os.path.join(working_directory, 'physical_features.done'), 'w')
endfile.close()
