...
def closest_effector(locus):
    for i in range(len(circular_contigs)):
        if locus in circular_contigs[i]:  # circular
            ...
            upstream_closest = 0
            for j in range(1, len(contig) - gene_place):
                if binary_l[gene_place + j] == 1:
                    upstream_closest = j
                    break
            if upstream_closest == 0:  # no effector is found upstream in the linear representation
                for j in range(gene_place):
                    if binary_l[j] == 1:
                        upstream_closest = j + len(contig) - gene_place
                        break
            downstream_closest = 0
            for j in range(1, len(contig)):
                if binary_l[gene_place - j] == 1:
                    downstream_closest = j
                    break
        ...
        if locus in linear_contigs[i]:
            ...
            for j in range(1, len(contig) - gene_place):
                if binary_l[gene_place + j] == 1:
                    ...
                    break
            downstream_closest = 0
            for j in range(1, gene_place):
                if binary_l[gene_place - j] == 1:
                    downstream_closest = j
                    break
            if upstream_closest != 0 and downstream_closest != 0:  # there are neighbor effectors both upstream and
                # downstream
                return min(upstream_closest, downstream_closest)
    ...
    for i in range(len(circular_contigs)):
        if locus in circular_contigs[i]:  # circular
            ...
            if gene_place >= k_neighbors:  # enough genes upstream
                ...
                more_to_count = k_neighbors - gene_place  # to add from the end which is further upstream to the start
                # (circular genome)
            ...
            if len(binary_l) - gene_place - 1 >= k_neighbors:  # enough genes downstream
                ...
                more_to_count = k_neighbors - (
                            len(binary_l) - gene_place - 1)  # to add from the start which is further
...
with open(os.path.join(working_directory, 'genome_organization_features.csv'), 'w', newline='') as f:
    ...
    for locus in locus_dic:
        l = [locus, closest_effector(locus)]
        for k in [5, 10, 15, 20, 25, 30]:
            l.append(effectors_in_neighbors(locus, k))
        csv_writer.writerow(l)
endfile = open(os.path.join(working_directory, 'genome_organization_features.done'), 'w')
endfile.close()
